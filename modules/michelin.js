...
exports.getAllRestaurant = (callback) => {
  pagesnumber(urlWeb).then((pages) => {
    ...
    let links = result.filter(x => x.isFulfilled).map(x => x.value);
...
function pagesnumber (url) {
  return new Promise(function (resolve, reject) {
    request.get(url, (err, res, body) => {
      if (err) {
        reject('page access error : trying to get the page number\n ' + err);
      }
      ...
      const pageNumberPath = '#panels-content-main-leftwrapper > div.panel-panel.panels-content-main-left > div > div > div > div.item-list-first > div > ul > li.mr-pager-item.mr-pager-placeholder';
...
function getRestaurantsLinks (page) {
  return new Promise(function (resolve, reject) {
    ...
    request.get(url, (err, res, body) => {
      if (err) {
        reject('page access error : trying to get all starred restaurant \n ' + err);
      }
      ...
      const itemsPath = '#panels-content-main-leftwrapper > div.panel-panel.panels-content-main-left > div > div > ul > li';
      let linksOnPage = [];
...
function getDescription (link) {
  return new Promise(function (resolve, reject) {
    request.get('https://restaurant.michelin.fr' + link, (err, res, body) => {
      ...
      const pagePath = 'body > div.l-page > div > div.l-main > div > div.panel-display.panels-michelin-content-layout.panels-michelin-2colsidebar.clearfix';
      ...
      if ($(pagePath)) {
        ...
        //if there is a photo we save the url
        if ($('#node_poi-photos-wrapper')) {
          item.photo = $(photo).children().children().children().text().split('"')[3];
        } else {
          //if not we save the url of the default photo for a restaurant
